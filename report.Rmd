...
Before we apply machine learning techniques to the training data, it is importand to carry out some exploratory analysis in an effort to address any problems with the data (for example, missing values).
...
```{r eda.na2}
training.na <- sapply(training_X, 
                      function(x) ((sum(is.na(x)))/dim(training_X)[1])>0.95)
...
remove_cols <- nearZeroVar(training.small, 
                           names = TRUE, 
                           freqCut = 2, 
                           uniqueCut = 20)
...
rm.var <- names(training.smaller) %in% 
...
corr.high <- findCorrelation(corr.mat, cutoff = 0.80)
...
mtext("Correlation Plot: Before and after highly correlated variables removed", side=3, outer=TRUE, line=-3)
```
...
myFolds <- createFolds(training_Y, k = 5)
myCtrl <- trainControl(
        index = myFolds,
        verboseIter = TRUE,
...
# KNN
modelknn <- train(training.X.uncorr,
                  training_Y,
                  method = "knn",
                  trControl = myCtrl
)
...
# RPART
modelrpart <- train(training.X.uncorr,
                  training_Y,
                  method = "rpart",
                  trControl = myCtrl
)

# Random Forest (automatic search for best mtry)
modelrf <- train(training.X.uncorr,
                 training_Y,
                 method="ranger",
                 trControl = myCtrl
                 )
...
modelglm <- train(training.X.uncorr,
                  training_Y,
                  method = "glmnet",
                  trControl = myCtrl
                  )
